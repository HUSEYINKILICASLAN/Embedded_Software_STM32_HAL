...
int main(void) {
	...
	MX_TIM3_Init();
	/* USER CODE BEGIN 2 */
	HAL_TIM_PWM_Start(&htim3, TIM_CHANNEL_1); //timer3 kanal1 den pwm uretimi baslatildi
	Set_PWM_Duty_Cycle(25);	//timer3 kanal1 den 25% duty cycle 1 KHz pwm uretildi
	HAL_TIM_IC_Start_IT(&htim1, TIM_CHANNEL_1);	//timer1 kanal1 interrupt modunda input capture modunda baslatildi
	/* USER CODE END 2 */
