...
#include "stdlib.h"    //atoi fonksiyonunu kullanabilmek icin yazildi
...
uint8_t blinkTime = 0;   //girilecek zaman degerini kontrol etmek icin yazildi
...
int main(void) {
	...
	while (1) {
		...
		/* USER CODE BEGIN 3 */
		if(interruptFlag){               //interruptFlag 1 olduysa ledi yak sonra bekle sonra interruptFlag 0 yap ve bekle
...
void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart) {
	if (huart->Instance == USART1) { //hangi UART modulundan geldigini ayirtetmek icin yazildi
		...
		} else if (strncmp(messageBuffer, "blink ", 6) == 0) { //gelen karakterler dizisinde 6 karakterlik dizi (blink ) olup olmadigi kontrol edildi
			blinkTime = atoi(&messageBuffer[6]);     //dizinin 6nci indeksindeki elemani integer yapar
			if (blinkTime > 0) {
				interruptFlag = 1;
				bufferIndex = 0;                //buffer index i sifirla
				memset(messageBuffer, 0, BUFFER_SIZE); //messageBuffer dizisini buffer size kadar sifirla
