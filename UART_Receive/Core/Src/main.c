...
uint8_t rxChar;        //bu degisken interrupt modda kullanilacak
char messageBuffer[BUFFER_SIZE];      //bu degisken interrupt modda kullanilacak
uint8_t bufferIndex;    //bu degisken interrupt modda kullanilacak
/* USER CODE END PV */
...
if (huart->Instance == USART1) {
	...
	if (strstr(messageBuffer, LED_ON_COMMAND) != NULL) {
		HAL_GPIO_WritePin(User_Led_GPIO_Port, User_Led_Pin, 1);
		bufferIndex = 0;
		memset(messageBuffer, 0, BUFFER_SIZE);
	} else if (strstr(messageBuffer, LED_OFF_COMMAND) != NULL) {
